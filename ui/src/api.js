...
export async function getCandidates(abbr, race) {
  const r = await fetch(`${API}/api/state/${abbr}/candidates?race=${race}`)
  if (!r.ok) throw new Error(`Failed to load candidates for ${abbr}`)
  return r.json()
}

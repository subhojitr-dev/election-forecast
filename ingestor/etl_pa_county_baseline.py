"""etl_pa_county_baseline.py — Pennsylvania baselines at COUNTY level.

WHY
  The live PA feed (pa_live_feed.py) reads electionreturns.pa.gov at COUNTY
  granularity (county name is a clean, exact-match join to our 67 PA counties
  — no name-matching). For the analytics to compare live-vs-baseline, the
  baseline must be at the SAME granularity. So we aggregate PA's existing
  precinct-level baselines into county-pseudo-precincts ("PA-{fips}-CTY") —
  the same pattern used for NC (etl_nc_county_baseline.py) and GA — and remove
  the now-superseded precinct-level rows for that race/year (county-level v1
  decision, 2026-06-30).

  Converts BOTH President 2024 and Senate 2024 (the 2024 general, electionid
  105, is PA's own live test target — it carries both races with mode splits).
...
def convert(c, race_type, year):
    state_full = c.execute("SELECT state FROM precincts WHERE state_abbr='PA' LIMIT 1").fetchone()[0]
    fips = {r[0]: r[1] for r in c.execute(
        "SELECT DISTINCT county, county_fips FROM county_rollup WHERE state='PA'")}

    rows = c.execute(
        """SELECT p.county, rh.candidate, rh.party, SUM(rh.votes) v
           FROM results_historical rh JOIN precincts p ON rh.precinct_id = p.id
           WHERE p.state_abbr='PA' AND rh.race_type=? AND rh.election_year=?
             AND p.id NOT LIKE 'PA-%-CTY'
           GROUP BY p.county, rh.candidate, rh.party""", (race_type, year)).fetchall()
    if not rows:
        n = c.execute("SELECT COUNT(*) FROM precincts WHERE id LIKE 'PA-%-CTY'").fetchone()[0]
        print(f"  {race_type} {year}: no precinct-level rows found — already county-level "
              f"or not loaded ({n} PA-CTY pseudo-precincts total). Nothing to do.")
        return
    ...
    c.execute("DELETE FROM results_historical WHERE race_type=? AND election_year=? "
              "AND precinct_id LIKE 'PA-%-CTY'", (race_type, year))
    ins_prec = ins_rh = 0
    for county, candidate, party, v in rows:
        ...
        if not c.execute("SELECT 1 FROM precincts WHERE id=?", (pid,)).fetchone():
            c.execute("""INSERT INTO precincts (id, state, state_abbr, county, county_fips, precinct_name)
                         VALUES (?,?,?,?,?,?)""",
                      (pid, state_full, "PA", county, cf, "COUNTY TOTAL"))
            ins_prec += 1
    ...
    c.execute("""DELETE FROM results_historical WHERE race_type=? AND election_year=?
                 AND precinct_id IN (SELECT id FROM precincts
                     WHERE state_abbr='PA' AND id NOT LIKE 'PA-%-CTY')""", (race_type, year))

    d = c.execute("""SELECT SUM(rh.votes) FROM results_historical rh JOIN precincts p ON rh.precinct_id=p.id
                     WHERE p.state_abbr='PA' AND rh.race_type=? AND rh.election_year=? AND rh.party='DEM'""",
    ...
    rr = c.execute("""SELECT SUM(rh.votes) FROM results_historical rh JOIN precincts p ON rh.precinct_id=p.id
                      WHERE p.state_abbr='PA' AND rh.race_type=? AND rh.election_year=? AND rh.party='REP'""",
                   (race_type, year)).fetchone()[0]
    nctot = c.execute("SELECT COUNT(*) FROM precincts WHERE id LIKE 'PA-%-CTY'").fetchone()[0]
    print(f"  {race_type} {year}: converted -> county level: +{ins_prec} pseudo-precincts, "
          f"{ins_rh} rows ({nctot} PA-CTY total)")
